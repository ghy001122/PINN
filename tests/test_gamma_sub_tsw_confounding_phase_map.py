...
from scripts.audit_gamma_sub_tsw_confounding_phase_map import run_tsw_confounding_phase_map
...
def test_tsw_confounding_phase_map_config_scope() -> None:
    config = yaml.safe_load(Path("configs/gamma_sub_tsw_confounding_phase_map.yaml").read_text(encoding="utf-8"))
    assert config["inverse"]["primary_parameter"] == "gamma_sub"
    assert config["sweep"]["T_sw_delta_K"] == [0.0, 0.04, 0.1, 0.2, 0.4, 1.0, 2.0]
    assert config["sweep"]["T_sw_prior_widths"] == [0.02, 0.05, 0.1, 0.2, 0.5, 1.0]
    assert "effective_T_sw_delta_K" in config["sweep"]["residual_delta_model"]


def test_tsw_confounding_phase_map_smoke(tmp_path: Path) -> None:
    ...
    config["sweep"]["T_sw_delta_K"] = [0.0, 0.2]
    config["sweep"]["T_sw_prior_widths"] = [0.1, 1.0]
    path = tmp_path / "config.yaml"
    path.write_text(yaml.safe_dump(config, sort_keys=False), encoding="utf-8")
    summary = run_tsw_confounding_phase_map(path)
    ...
    assert summary["relative_error_matrix"]["value_key"] == "relative_error"
    for row in summary["rows"]:
        ...
        assert "effective_T_sw_delta_K" in row
