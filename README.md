...
This repository is a reproducible Python 3.11 research codebase and evidence package for a Q2 SCI delivery objective. Phase 1 Checkpoint A contains an independent, conservative Qiu-inspired VO2 single-device 2.5D implementation, behavior tests, bounded smoke, and a locked 96-case manifest. A non-voting v7 vertical-reference repair is now `failed_but_informative`, so Checkpoint B remains blocked. The formal campaign has not run (`formal_execution_count=0`); no Phase 1 scientific result, positive PINN result, inverse result, or experimental validation exists.
