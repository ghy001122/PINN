# Discussion V1

The main scientific point is not optimizer novelty or full-field recovery. The point is that sparse-port inverse diagnosis must first pass an identifiability audit. In this benchmark, the defensible route is to reduce the target space and estimate \(\gamma_{\mathrm{sub}}\) only under fixed or tightly bounded priors.

The new quasi-2D preflight increases physical depth by documenting how a laterally resolved phase-transition thin-film extension could be constructed. It remains forward-only. Because the unknown field dimension grows sharply in two dimensions, sparse terminal current cannot identify full 2D fields without additional observability.

F-SPS-PINN evidence remains supplementary or future work. Current small-run evidence does not justify a performance-superiority claim.

## Supplementary Stiffness And Phase-Field Defense

The stiffness-continuation audit supports a limited reviewer-defense point: as the synthetic phase-transition width is reduced, the autograd residual proxy increases sharply, so a phase-transition cliff is a valid numerical stress case. Continuation reduces the residual proxy in this preflight, while Fourier features are not uniformly beneficial. This should be discussed as stability motivation, not as proof of F-SPS-PINN superiority.

The phase-field alignment smoke benchmark passes as a full-field-anchor Allen-Cahn mobility inversion sanity check. It aligns the manuscript with phase-field inverse-PINN literature, but it does not alter the main sparse-port `gamma_sub` claim.

## Final Stiffness And Quasi-2D Placement

The stiffness and quasi-2D results increase reviewer-defense depth without changing the core contribution. The paper should present them as physics-consistent extensibility and observability-boundary evidence. The main story remains one-dimensional sparse-port calibration-gated `gamma_sub` inversion.

The correct interpretation is: calibration control is the dominant condition in the existing synthetic comparisons, while Figure 5 cannot causally isolate protocol gain because calibration error and waveform settings vary together; 2D inverse diagnosis requires additional observability; continuation helps residual stiffness in a preflight but is not a full STL-PINN result.

## Discussion Addendum: Claim-Gated 2D And Stiffness Evidence

The new audits convert prior narrative gaps into bounded evidence. Reduced 2D forward computability is now supported by a finite benchmark, but this does not alter the main one-dimensional `gamma_sub` claim. The observability ladder is especially important: terminal-only 2D inverse diagnosis fails, while augmented port/proxy observations support only low-dimensional parameter diagnosis. This gives the manuscript a defensible path to discuss 2D extensibility without overclaiming full-field recovery.

The stiffness-aware algorithm benchmark supports the weaker statement that continuation, scale-aware weighting, and mini-STL-style transfer mitigate stiffness-induced degradation in a synthetic residual-proxy setting. It does not support full STL-PINN reproduction or F-SPS superiority.

## Public-Data And Full-PINN Failure Boundary

The VO2 audit shows why source-code agreement is weaker than numerical convergence. Matching the author implementation at its declared step does not establish a step-converged reference, and therefore cannot justify a repository refit or protocol-identifiability story. M37R also shows that nominal solver parity is weaker than differentiable local observability: after fixing the event window, a 1% thermal-capacitance perturbation at 15 V changes the first post-window event type and stops the central-difference Jacobian contract. The source paper's parameter-optimization statement also prevents 13 V from being called strictly independent external validation.

The N0 result similarly separates an architecture contract from learned evidence. Hard constraints and a manufactured solution can pass while out-of-sample residuals and terminal trajectories fail. This justifies the rule that trajectory fit alone cannot open an inverse claim and that solver/PINN sensitivity comparison must remain blocked until forward residual fidelity is demonstrated. The calibration-gated `gamma_sub` evidence remains the safe inverse mainline; the bounded neural-repair route is now closed.

The bounded repair sharpens that conclusion. Correcting the electrode orientation and replacing a finite-band proxy with exact layer traces materially improves the potential and interface diagnostics, but local strong-form residuals still do not close terminal-current, defect-mass, or energy ledgers. This rules out a simple interface-only explanation. Any further neural forward attempt must change the numerical residual contract toward solver-consistent face/control-volume or weak conservation, not repeatedly tune the completed strong-form branch.

The control-volume sequence separates three evidence levels. Operator parity and preflight consistency pass; the historical b380 training record then aborts without a scoreable trajectory; and the instrumented v3r replay finally exposes a scoreable post-Adam state that fits the port narrowly but violates residual, field, interface-flux, and conservation gates by large margins. The same checkpoint also reproduces a strong-Wolfe non-finite parameter failure. Thus, neither a correct differentiable operator, a successful preflight, nor a terminal-trajectory fit establishes full-PINN conservation fidelity. The locked `20x` stop rule makes recovery ineligible, so preserving the failure and closing N0 is more defensible than altering schedule, normalization, or supervision after inspection.

M33 tests the remaining bounded alternative rather than tuning that closed optimizer path: an explicit first-order mixed state--flux representation with grouped augmented-Lagrangian feasibility training. It reduces the dominant thermal residual and both interface-flux errors by large factors, demonstrating that residual representation was part of the numerical difficulty. It does not solve the coupled problem. The conservation and phase/current multipliers reach their caps, the constitutive group does not converge, the port and several fields regress, and independent full-trajectory ledgers remain failed even though the training-time explicit-head ledger RMS values pass. The important reviewer-facing conclusion is that local or training-grid conservation penalties are not substitutes for independent trajectory conservation. Under the preregistered stop rule, this closes new full-PINN training exploration and leaves the complete PINN as an explicit architecture/failure scaffold rather than a positive trained method result.

The solver-first SID/EC-OQ audit provides a parallel caution. Isolated rank consistency, information ratios, or large raw principal angles cannot support a protocol-geometry claim when the finite-difference Jacobian fails convergence, an event window degenerates to the full trace, and the direction is not stable across at least three protocols and a parameter neighborhood. Accordingly, the current implementation is rejected and inactive rather than renamed around a negative result. This is an implementation-level stop, not permanent falsification of every protocol-dependent identifiability hypothesis; a revisit would require a new numerical derivative and event-window contract.

## CPCF Supersession, CEBA Abstention, And Physical-Upgrade Boundary

The stricter semantic audit shows that CPCF cannot answer even the negative frontier question: response-surface labels, fresh-anchor waveforms, solvers, grids, objectives, candidate sets, units, replicate semantics, and Pareto eligibility are not aligned. Its historical files are preserved, but the scientific interpretation is superseded. This distinction matters because an invalid proxy contract cannot be converted into evidence that a frontier does not exist.

CEBA repairs the implementation contract and exactly reproduces all six historical source anchors. The scientific hypothesis still fails at the next gate: a deliberately conservative ambiguity set crosses the recoverable/nonrecoverable gamma classes even at zero switching-temperature error. The correct conclusion is not that calibration can never work; it is that this preregistered CEBA scoring contract cannot estimate a boundary and must abstain. Trajectory reuse makes that negative conclusion inexpensive without weakening the direct-solver source semantics.

The subsequent semantic audit further limits that interpretation: CEBA abstention is computed with the known synthetic truth and is sensitive to the candidate-grid span. It cannot be presented as an operational refusal rule. SCIS removes truth access from inference and achieves its pre-registered nominal pooled and worst-candidate coverage gates, but accepts every severe `2 K` mismatch case while the point estimate is always outside the `15%` success radius. This is precisely the unsafe failure mode a refusal mechanism was meant to prevent. The correct manuscript use is therefore a negative reviewer-defense result: nominal coverage alone did not confer model-mismatch detection.

Figure 5 is likewise retained only after semantic downgrade. The 720 synthetic solver cases support the conditional performance of a named bundled calibrated configuration; they do not separate waveform/protocol effects from calibration-error reduction, and they do not establish an optimal experimental protocol.

The literature red team also blocks novelty by component. PirateNet, DB-PINN, SOAP/NNCG, XPINN, DeepONet, differentiable event models, DWR/QoI sampling, enthalpy PINNs, VO2 hysteresis, and thermal-neuristor compact models all have precedent. The only retained neural hypothesis is their evidence-gated combination in a complete multidomain model with identifiable-QoI allocation, physical event ledgers, independent solver/field/interface/conservation checks, and refusal under ambiguity. H1 latent heat, H2 history state, and H3 two-dimensional geometry remain discrepancy-triggered upgrades rather than fit-improving free parameters.
