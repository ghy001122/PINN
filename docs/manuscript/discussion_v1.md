...
## CPCF And Physical-Upgrade Boundary

The CPCF pilot shows why nondominance alone is insufficient. Five points are stable within the sampled table, yet the fresh solver anchors violate the pre-existing response-surface tolerance and no candidate clears the locked `20%` improvement or directionality gates. Therefore the apparent sampled chain cannot support a minimum-resource or cost-optimality statement. Its value is negative: it demonstrates that a resource prescription must first survive an independent-solver consistency check.

The literature red team also blocks novelty by component. PINN preconditioning, profile likelihood, Bayesian experimental design, adaptive PINN point selection, enthalpy PINNs, VO2 hysteresis, and thermal-neuristor compact models already exist. H1 latent heat, H2 history state, and H3 two-dimensional geometry are retained only as evidence-triggered hierarchy upgrades. Adding any of them to improve fit without an external discrepancy, conservation derivation, and independent solver would weaken rather than strengthen the manuscript.
