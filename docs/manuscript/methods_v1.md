...
M33 keeps that frozen cell, material, electrical, observation, and scoring contract but predicts explicit face-centered defect and heat fluxes together with (c_i,T_i,m_i). The second-order transport blocks are replaced by first-order constitutive connection residuals and flux-divergence conservation residuals. Constitutive, conservation, phase/current, IC/BC, interface, and global-ledger constraints have separately preregistered augmented-Lagrangian multipliers and penalties. The shared trunk is reduced so that the 5632-parameter mixed model differs from the 5501-parameter v3r baseline by only `2.38%`. The single seed uses 600 feasibility and 900 coupled Adam steps, exactly `1.25x` the v3r Adam budget; no labels, continuation, L-BFGS, optimizer search, seed expansion, or post-result weight change is allowed. All scoring returns to frozen H0 and the original 400-by-31 synthetic test grid.
