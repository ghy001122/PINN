...
## R12. Bounded CPCF Pilot Rejection Boundary

Status: `failed_but_informative`. The valid preregistered pilot completed `48` cases over `12` operating points with `8` fresh solver anchors and `48` solver forward evaluations. Five sampled nondominated points meet the bootstrap-stability definition, so gate b passes. Gate a fails: fresh-anchor mean absolute discrepancy is `0.299600` versus the locked `0.0537361`, classification agreement is `0.625` versus `0.883333`, and the disagreement count is `3` versus `0`. Gate c fails because no candidate reaches the locked `20%` resource or risk improvement; gate d therefore has no improvement-direction interval excluding zero. Full expansion was not triggered. The operating-point table and Pareto plot are supplementary rejection evidence, not a stable, monetary, experimental, or device-general cost frontier.
