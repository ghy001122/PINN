...
## R15. Mixed State--Flux Feasibility-First MVE

Status: preflight `supported`; trained result `failed_but_informative`. The 5632-parameter contract passes all `16/16` preregistered no-training checks, including constant equilibrium, exact frozen-FVM parity, flux sign/unit/scale, one-sided interface traces, independent ledgers, automatic/finite-difference gradients, tamper controls, finite multipliers, and frozen hashes. The only training run completes all 1500 Adam steps on seed `20260715` without non-finite values.

The mixed contract materially reduces selected v3r failures: (r_T) falls from `56586.4844` to `1048.3698`, the interface heat-flux NRMSE95 from `33211.9341` to `267.6582`, and the interface defect-flux NRMSE95 from `23.8718` to `9.63365`. These remain respectively about `1.05e5`, `5.35e3`, and `193` times their unchanged gates. Port NRMSE95 degrades from `0.0955475` to `0.116783`; the constitutive RMS values are `0.795344` for heat and `0.0822844` for defect; the state-trajectory energy and defect ledger gates remain `0.999955` and `1.0`; and only `10/20` matched v3r metrics are non-worse. Explicit-head training-ledger RMS values pass, but independent full-trajectory ledgers fail, so the training ledger cannot be promoted as conservation evidence. M33 therefore does not establish a feasible full-PINN forward solution, and N1-N3 remain unrun.
