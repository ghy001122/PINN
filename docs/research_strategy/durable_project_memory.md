...
- High-risk ideas should be explored through bounded audits when the active phase authorizes them; manuscript wording remains claim-gated.
...
`Q2_SCI_DELIVERY_MODE` is the persistent delivery goal. The degradable ladder is R1 `HysGeo-Hybrid-PINN` as the minimum route, R2 `GeoPhase-HomoMoE-PINN` as the preferred method upgrade, and R3 event-aligned observable-subspace/OQ only after solver and PINN sensitivity gates. The versioned Phase 1-v2 reference contract uses the independent Qiu-inspired real x-y plane plus the source-scale-preserving locally distributed S2 nominal thermal closure. S1 diffusive memory is non-blocking model-form sensitivity unless independent same-device thermal evidence passes its separate gate. The v6-v8 material-stack/K-state route is terminal `failed_but_informative`. Frozen 1D, constrained `gamma_sub`, OASIS, public-source, and failed complete-PINN work remains historical baseline and reviewer-defense evidence; it cannot silently become evidence for Phase 1-v2 or R1-R3.

## Claim Vocabulary

Use exactly: `supported`, `qualified_supported`, `failed_but_informative`, `forbidden`.

## Recurring Pitfalls Index

1. Synthetic evidence described as measured data.
2. Engineering priors described as measurements.
3. Literature trends described as quantitative validation.
4. Frozen GT changed to improve a result.
5. Post-hoc threshold relaxation.
6. Failed seeds or parameter blocks hidden by averages.
7. Port fit equated with hidden-field recovery.
8. Integrated conductance treated as unique field information.
9. Local Jacobian rank described as global identifiability.
10. Linearized inverse described as nonlinear prior-domain recovery.
11. Response-surface interpolation described as new simulator solves.
12. Preflight or proxy described as actual training.
13. Finite/backward/loss decrease described as performance success.
14. Smoothness regularization described as a PDE residual.
15. Field anchors omitted from the method label.
16. Target or holdout leakage into features or bases.
17. Local and global boundary coordinates mixed.
18. Interface residual built from the same expression on both sides.
19. Electrical and thermal device graphs conflated.
20. Material-family mechanisms or transition temperatures mixed.
21. Ad-hoc local NDR substituted for electrothermal feedback.
22. Threshold activation forced by unlabelled parameter shifts.
23. Mean-sigma port proxy described as an electrostatic solve.
24. Single-trace terminal inverse confused with active multi-protocol inverse.
25. Low-rank or latent recovery described as arbitrary full-field recovery.
26. Mini-STL or warm start described as full STL reproduction.
27. Conditional Fourier gain described as universal superiority.
...
Applicable AGENTS -> current Git and outputs -> `CODEX_CONTEXT.md` plus `active_phase.md` -> `PROJECT_STATE.md` and `NEXT_ACTIONS.md` -> canonical handoff and archives -> local Codex memory.
