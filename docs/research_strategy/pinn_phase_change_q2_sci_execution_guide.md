...
“排列组合＋魔改”是有来源约束和证据门控的研究合成方法，而不是要求每个组成模块都必须首创或改写。允许直接迁移并正确引用模块，也允许只对其中一部分做物理变量、材料机制、几何、边界／接口、控制流、协议、solver coupling、refusal 或证据合同适配。

可论证的组合增量可以位于适配、接口、流程、功能组合、验证合同或适用域；底层模块已有文献不是否定组合成果的理由。Headline 组合必须用最小区分性证据比较最强直接基线、一个关键接口／模块消融和完整组合，但 idea/MVE 阶段不要求对所有排列组合做全量消融，也不要求每个误差指标都击败所有基线。

组合价值可由新功能、新适用域、分支保持、集合输出／拒判、随机访问、守恒闭合、可组合性、速度、数据效率、稳定性、鲁棒性或新的物理认识建立。来源、许可、直接迁移／精确修改、模块接口、证据边界和禁止 claim 必须清楚；不得为制造“原创感”增加无用复杂度，也不得把一个 frozen NO-GO 自动升级为对全部新组合的永久禁令。任何新组合仍须服从当前 active phase、前置 gate、数据边界和预算；该方法论本身不授权重新执行已停止路线。
...
- 几何：长度、宽度、电极搭接、相邻间距；
- 物理：\(G_\theta,C_\theta,T_c^\uparrow,T_c^\downarrow,w_T,\tau_s\)；S1 参数不进入 nominal 数据集；当前 Phase 1 未实现接触电阻，因此 \(R_c\) 只有在独立接触模型实现并验证后才能加入；
- 协议：偏压、脉冲、\(R_L,C_p\)；
- 工况：静息、振荡、锁定；双器件只保留零耦合副本极限。非零双器件耦合必须等待显式衬底场或独立验证的被动非局域核及新合同。
...
当前 S2 nominal 没有独立垂向状态，因此不得输出或宣称恢复
\(z_1,z_2\)。\(V_d\) 可由全局电路 head 或与端口层一致的隐式电路层表示。
