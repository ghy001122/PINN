...
| Bundled calibrated-configuration performance | `scripts/audit_gamma_sub_calibrated_sequential_protocol_validation.py`; `scripts/audit_prompt32_semantics.py` | `outputs/tables/gamma_sub_calibrated_sequential_protocol_validation_summary.json`; `outputs/tables/prompt32_figure5_semantic_audit.json` | Figure 5 / Table 6 | The named bundle has the reported conditional synthetic performance; calibration and waveform effects are not isolated | Protocol gain, optimality, or experimental validation |
| SCIS nominal coverage and refusal failure | `scripts/run_gamma_sub_scis.py` | `outputs/tables/gamma_sub_scis_summary.json` | Supplementary Figure S-SCIS | Nominal discrete-grid coverage passes, but severe mismatch is not refused | Operational or experimental coverage/refusal certificate |
