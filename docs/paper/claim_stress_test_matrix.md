...
| Full 1D PINN forward gate passes | `outputs/tables/n0_cv_e_v3r_forensic_resolution.json`; `outputs/tables/m33_mixed_flux_final_summary.json` | v3r port NRMSE95 `0.0955475`; M33 reduces thermal residual and explicit interface-flux errors without labels or non-finite values | M33 port is `0.116783`; constitutive/PDE/field/flux/global-ledger gates fail and only `10/20` v3r metrics are non-worse | reliable full PINN, conservation, sensitivity fidelity, or inverse success |
...
| CPCF establishes or rejects a resource frontier | `outputs/tables/prompt31_cpcf_semantic_audit.json`; immutable historical CPCF artifacts | The audit accounts for 48 proxy votes, eight diagnostic anchors and zero direct-solver voting rows. | Protocol, solver, candidate, objective, unit, bootstrap and Pareto contracts are invalid for a scientific vote. | Any conclusion about frontier existence/absence, cost optimality, or resource prescription. |
| CEBA establishes a direct-solver boundary | `outputs/tables/gamma_sub_ceba_parity_summary.json`; `outputs/tables/gamma_sub_ceba_pilot_summary.json` | `6/6` parity anchors pass; 72 scoring cases reuse 36 trajectories. | The ambiguity rule abstains at `delta_T_sw_K=0`, so no success/failure bracket or refinement exists. | Estimated boundary, optimal protocol, or relaxed abstention after inspection. |
