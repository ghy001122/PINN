...
| N0/M33 trained forward evidence | `failed_but_informative` | `outputs/tables/n0_cv_e_v3r_forensic_resolution.json`; `outputs/tables/m33_mixed_flux_final_summary.json` | v3r's port gate passes but its physics gates fail. M33 changes to a first-order mixed-flux/AL contract and improves selected thermal/flux errors, yet fails port, constitutive, PDE, field, interface-flux, global-ledger, and no-regression gates. New full-PINN training is stopped. | Reliable full-PINN forward, conservation, sensitivity, or inverse readiness. |
| Solver-first SID/EC-OQ discovery | `failed_but_informative` | `outputs/tables/sid_ec_oq_summary.json` | Derivative, event-window, stability, and dual-geometry gates fail; the current implementation is rejected and inactive, without permanently falsifying the broader hypothesis. | Protocol rank/rotation, SID, EC-OQ, or quotient superiority. |
| CPCF semantic vote | `forbidden` for frontier inference; `failed_but_informative` as software audit | `outputs/tables/prompt31_cpcf_semantic_audit.json` | Protocol/solver/grid/candidate/objective/unit/replicate/Pareto contracts are non-equivalent; CPCF is non-voting. | Any positive or negative resource-frontier conclusion, cost optimality, or laboratory requirement. |
| Direct-solver CEBA boundary | `failed_but_informative` | `outputs/tables/gamma_sub_ceba_parity_summary.json`; `outputs/tables/gamma_sub_ceba_pilot_summary.json` | Source parity passes `6/6`, but the locked ambiguity rule yields no success/failure bracket and no refinement vote. | Estimated `delta_T_sw_star`, optimal protocol, or post-hoc threshold relaxation. |
